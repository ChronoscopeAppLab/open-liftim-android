    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (action == Intent.ACTION_MY_PACKAGE_REPLACED) {
            JobRequest.Builder(UpdateAccountInfoJob.TAG)
                    ...
                    .build()
                    .scheduleAsync()
            val nextMidnight = DateTime.now().withHourOfDay(0).plusDays(1)
            JobRequest.Builder(NotificationRegistererJob.TAG)
                    .setExact(nextMidnight.millis - DateTime.now().millis)
                    .build()
                    .schedule()
        }
    }
